...
export default class Base extends React.Component {
  ...
  constructor(props) {
    super(props)
    this.tranDict = util.getTransitionKeys();
    this.space =  props.space || 0;
    this.touchoffset = this.props.touchoffset || 50;
  ...
  init(props,isReciveProps){
    ...
    this.canTouchToNext = true;
    this.canTouchToPre = true;
    ...
    if(this.isHorizontal){
      this.wrapperSize = document.body.offsetWidth;
      if(this.props.itemWidth){
        this.wrapperSize = this.props.itemWidth;
      }
    }else{
      this.wrapperSize = 200;
    }
  ...
  goNextByStep(step){
    ...
    this.swipUseAnimate();
    this.setState({offset:step*(0-this.wrapperSize-this.space)});
  ...
  goPreByStep(step){
    ...
    this.isIntransition = true;
    this.swipUseAnimate();
    this.setState({offset:step*(this.wrapperSize+this.space)});
  ...
  toggleEnableGoNext(enable){
      this.canTouchToNext = enable;
  }

  toggleEnableGoPre(enable) {
      this.canTouchToPre = enable;
  }
  ...
  _renderItem(params){
    ...
    if(sourceIndex!==-1){
      if((this.props.lazyrender&&index===1&&!childrenItem)||!this.props.lazyrender){
        ...
        childrenItem = this.props.renderItem({swiper:this,index:sourceIndex,data:this.props.data[sourceIndex]});
        }
      }
    }
  ...
  render() {
    ...
    if(this.wrapperSize){
      ...
      for(var i=0;i<len;i++){
        ...
        var itemStyle = {};
        var v = ((i-1)*this.space+(i-1)*this.wrapperSize+this.state.offset);
        var vstr = this.isHorizontal? v +"px,0,0":"0,"+v+"px,0";
        itemStyle[this.tranDict.transform] = "translate3d("+vstr+")"
        if(this.props.itemWidth){
          itemStyle.width = this.wrapperSize+"px";
        }
        if(!this.animate){
  ...
  return (
    <div style={this.props.style||{}}  {...toucheEvent} className={classNameArr.join(" ")}>
        {children} 
